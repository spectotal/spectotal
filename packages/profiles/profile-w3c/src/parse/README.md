...
- markdown include recognition happens in `../compose/compose-document.ts`
- generic recursion, cycle checks, and fragment splicing happen in `@spectotal/source-compose`
- mdast parsing starts only after composition has produced ordered fragments
...
```mermaid
flowchart TD
  A["W3C source"] --> B["compose-document.ts\nsplit markdown into content/include parts"]
